...
// Plan to handle the received proposal object
+proposal(Name, Type, Price, Est_up, Est_down)
<- .println("Received proposal: name = ", Name, ", type = ", Type, ", price = ", Price, ", Estimated increase = ", Est_up, ", Estimated decrease = ", Est_down);
...
+proposal_response(name(Name), type(Type), price(Price), est_up(Est_up), est_down(Est_down), response(Response))[source(A)]: Response == 0 
...
<- .println("Received response for proposal: name = ", Name, ", type = ", Type, ", Price = ", Price);
    ...
    .send(risk, tell, proposal_response(name(Name), type(Type), price(Price), est_up(Est_up), est_down(Est_down), response(Response))).
